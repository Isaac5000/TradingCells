...
def run_measurement(
    ...
    try:
        ...
        for command in setup_commands:
            response = rcon_command(rcon_port, command.removeprefix("/"))
            if any(error in response for error in COMMAND_ERRORS):
                raise RuntimeError(f"Minecraft rechazó {command!r}: {response}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Run a server command after startup and before warmup; may be repeated.",
    )
    parser.add_argument(
        "--template-directory",
        type=Path,
...
def main() -> None:
    ...
    definition = scenario_definition(tools_directory, "server", args.scenario)
    if definition.get("template_required") and not args.template_directory:
        raise ValueError(
            f"El escenario {args.scenario!r} requiere --template-directory para preservar "
            "inventarios, entidades, configuración y estado exactos"
        )
    workload = args.workload or Path(__file__).with_name(f"{args.scenario}.txt")
    workload = workload.resolve()
    template_argument = args.template_directory.resolve() if args.template_directory else None
    if template_argument:
        if not (template_argument / "world" / "level.dat").is_file():
            raise ValueError(f"La plantilla {template_argument} no contiene un mundo de Minecraft")
        validate_manifest(
            template_argument,
            "server",
            args.scenario,
    ...
    elif not workload.is_file():
        raise ValueError(f"No existe la carga reproducible {workload}")

    timestamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    output = (args.output_directory or root / "build" / "performance" / "results" / args.scenario / timestamp).resolve()
    output.mkdir(parents=True, exist_ok=False)
    template = template_argument or output / "template"
    commands = [] if template_argument else read_workload(workload)
    ...
    if template_argument:
        ...
        write_manifest(
            ...
            notes="Generated by run_server_benchmark.py from the versioned workload.",
        )
    ...
    metadata = {
        ...
        "workload": str(workload) if workload.is_file() else "",
        "template_directory": str(template),
        "template_manifest": manifest,
        "template_manifest_name": MANIFEST_NAME,
