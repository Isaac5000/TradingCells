...
public final class ZombieFarmMenu extends AbstractContainerMenu {
    public static final int WIDTH = ZombieFarmMenuLayout.WIDTH;
    public static final int HEIGHT = ZombieFarmMenuLayout.HEIGHT;
    ...
    public static final int PLAYER_INVENTORY_X = ZombieFarmMenuLayout.itemX(
            ZombieFarmMenuLayout.PLAYER_INVENTORY_X
    );
    public static final int PLAYER_INVENTORY_Y = ZombieFarmMenuLayout.itemY(
            ZombieFarmMenuLayout.PLAYER_INVENTORY_Y
    );
    public static final int PLAYER_HOTBAR_Y = ZombieFarmMenuLayout.itemY(
            ZombieFarmMenuLayout.PLAYER_HOTBAR_Y
    );
    ...
    private static final int MACHINE_SLOT_COUNT = ZombieFarmBlockEntity.CONTAINER_SIZE;
    ...
    public ZombieFarmMenu(int containerId, Inventory inventory) {
        this(containerId, inventory, new SimpleContainer(MACHINE_SLOT_COUNT), new SimpleContainerData(9));
    }

    public ZombieFarmMenu(
            int containerId,
            Inventory inventory,
            Container container,
            ContainerData data
    ) {
        super(ZombieFarmRegistrationAdapter.MENU.get(), containerId);
        checkContainerSize(container, MACHINE_SLOT_COUNT);
        checkContainerDataCount(data, 9);
        this.container = container;
        this.data = data;
        this.registries = inventory.player.registryAccess();
        this.catalogTargets = ZombieFarmTargetCatalog.targets();
        if (container instanceof ZombieFarmBlockEntity farm) {
            selectedTargetId = farm.selectedTargetId();
            disabledDynamicLoot.addAll(farm.disabledDynamicLoot());
        } else {
            selectedTargetId = ZombieFarmTargetCatalog.id(ZombieFarmKind.ZOMBIE);
        }

        addSlot(new WorkerSlot(container, ZombieFarmBlockEntity.WORKER_SLOT, WORKER_SLOT_X, INPUT_SLOT_Y));
        addSlot(new SwordSlot(container, ZombieFarmBlockEntity.SWORD_SLOT, SWORD_SLOT_X, INPUT_SLOT_Y));
        for (int index = 0; index < ZombieFarmBlockEntity.OUTPUT_SLOT_COUNT; index++) {
            addSlot(new OutputSlot(
                    container,
                    ZombieFarmBlockEntity.FIRST_OUTPUT_SLOT + index,
                    outputSlotX(index),
                    outputSlotY(index)
            ));
        }
    ...
    public ZombieFarmKind selectedKind() {
        return ZombieFarmTargetCatalog.staticKind(selectedTargetId);
    }
    ...
    public boolean isLootEnabled(ZombieFarmLoot loot) {
        return isLootAvailable(loot) && (data.get(3) & loot.bit()) != 0;
    }

    public boolean isLootAvailable(ZombieFarmLoot loot) {
        return availableLootOptions().contains(loot);
    }

    public List<ZombieFarmLoot> availableLootOptions() {
        if (ZombieFarmTargetCatalog.isStaticTarget(selectedTargetId)) {
    ...
    public List<ItemStack> dynamicLootOptions() {
        return selectedTarget().lootItemIds().stream()
                .map(id -> BuiltInRegistries.ITEM.getOptional(id).orElse(null))
                .filter(item -> item != null && ZombieFarmTargetCatalog.category(item) == null)
                .map(ItemStack::new)
                .toList();
    }
    ...
    public boolean hasWorker() {
        return getSlot(ZombieFarmBlockEntity.WORKER_SLOT).hasItem();
    }

    public boolean hasSword() {
        return getSlot(ZombieFarmBlockEntity.SWORD_SLOT).hasItem();
    }
    ...
    public boolean clickMenuButton(@NonNull Player player, int buttonId) {
        ...
        if (buttonId >= SELECT_KIND_BUTTON_BASE
            ...
            if (container instanceof ZombieFarmBlockEntity farm) {
                ...
                data.set(2, ZombieFarmTargetCatalog.staticKind(target).ordinal());
            }
            return true;
        }
        if (buttonId >= TOGGLE_LOOT_BUTTON_BASE
                && buttonId < TOGGLE_LOOT_BUTTON_BASE + ZombieFarmLoot.values().length) {
            ZombieFarmLoot loot = ZombieFarmLoot.values()[buttonId - TOGGLE_LOOT_BUTTON_BASE];
