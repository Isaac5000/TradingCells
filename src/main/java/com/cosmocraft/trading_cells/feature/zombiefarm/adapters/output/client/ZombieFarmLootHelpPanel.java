...
/** Contextual live-yield panel owned by the normal Zombie Farm screen, never by REI. */
final class ZombieFarmLootHelpPanel {
    ...
    private int scroll;
    private ZombieFarmKind cachedKind;
    ...
    void drawPanel(
        ...
        ZombieFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.zombie_help.title"),
                x + 28,
                x + PANEL_WIDTH - 28,
                y + 2,
                HEADER_HEIGHT - 2,
        ...
        if (entries.isEmpty()) {
            ZombieFarmTextRenderer.centered(
                    graphics,
                    font,
                    Component.translatable("gui.trading_cells.zombie_help.no_loot"),
                    x + PANEL_PADDING,
                    x + PANEL_WIDTH - PANEL_PADDING,
                    firstRowY + 10,
                    ROW_HEIGHT,
    ...
    private void refresh(ZombieFarmMenu menu) {
        ZombieFarmKind kind = menu.selectedKind();
        ...
        for (ZombieFarmLootAdapter.PreviewOutput preview : ZombieFarmLootAdapter.previewOutputChances(kind)) {
            if (!menu.isLootEnabled(preview.loot())) {
                continue;
            }
            ZombieFarmDropRules.BaseDrop drop = ZombieFarmLootAdapter.currentCycleDrop(
                    kind,
                    preview.loot(),
                    preview.stack(),
                    looting,
    ...
    private static int dynamicState(ZombieFarmMenu menu) {
    ...
    private static Component status(
        ...
        if (!enabled) {
            return Component.translatable("gui.trading_cells.zombie_help.paused");
        }
        if (!worker) {
            return Component.translatable("gui.trading_cells.zombie_help.missing_villager");
        }
        if (!sword) {
            return Component.translatable("gui.trading_cells.zombie_help.missing_sword");
        }
        if (empty) {
            return Component.translatable("gui.trading_cells.zombie_help.no_loot");
        }
        return Component.translatable("gui.trading_cells.zombie_help.stats", looting, kills);
    }
        ...
        if (hovered) {
            Component amountTooltip = entry.minimumAmount() == entry.maximumAmount()
                    ? Component.translatable(
                            "gui.trading_cells.zombie_help.amount_exact",
                            entry.maximumAmount()
                    )
                    : Component.translatable(
                            "gui.trading_cells.zombie_help.amount_range",
                            entry.minimumAmount(),
                            entry.maximumAmount()
                    );
            List<Component> tooltip = entry.dynamic()
                    ? List.of(
                            entry.stack().getHoverName(),
                            Component.translatable("gui.trading_cells.zombie_help.dynamic")
                    )
                    : List.of(
                            entry.stack().getHoverName(),
                            Component.translatable("gui.trading_cells.zombie_help.chance", probability),
                            amountTooltip
                    );
