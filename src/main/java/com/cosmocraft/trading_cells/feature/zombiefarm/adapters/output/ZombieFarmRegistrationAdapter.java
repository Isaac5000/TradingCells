package com.cosmocraft.trading_cells.feature.zombiefarm.adapters.output;

import com.cosmocraft.trading_cells.feature.zombiefarm.adapters.input.ZombieFarmBlock;
...
public final class ZombieFarmRegistrationAdapter {
    public static final String ID = "zombie_farm";

    public static final DeferredBlock<ZombieFarmBlock> BLOCK = Registration.BLOCKS.register(ID, () ->
            new ZombieFarmBlock(MachineBlockProperties.villager(ID))
    );

    public static final DeferredHolder<BlockEntityType<?>, BlockEntityType<ZombieFarmBlockEntity>> BLOCK_ENTITY =
            Registration.BLOCK_ENTITY_TYPES.register(ID, () ->
                    new BlockEntityType<>(ZombieFarmBlockEntity::new, BLOCK.get())
            );
    ...
    public static final DeferredHolder<MenuType<?>, MenuType<ZombieFarmMenu>> MENU =
            Registration.MENU_TYPES.register(ID, () ->
                    new MenuType<>(ZombieFarmMenu::new, FeatureFlags.VANILLA_SET)
            );

    private ZombieFarmRegistrationAdapter() {
    }

    public static void load(IEventBus modEventBus) {
        modEventBus.addListener(ZombieFarmRegistrationAdapter::onRegisterCapabilities);
    }
