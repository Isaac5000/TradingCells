...
public final class ZombieFarmScreen extends AbstractContainerScreen<ZombieFarmMenu> {
    ...
    private static final ZombieFarmGuiThemeColors COLORS = ZombieFarmGuiThemeColors.resolve();
    ...
    private final ZombieFarmLootHelpPanel lootHelp = new ZombieFarmLootHelpPanel();
    private Button lootHelpButton;
    private boolean kindListOpen;
    private int kindScroll;
    private int lootScroll;

    public ZombieFarmScreen(ZombieFarmMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, ZombieFarmMenu.WIDTH, ZombieFarmMenu.HEIGHT);
        titleLabelY = ZombieFarmScreenLayout.HEADER_TEXT_Y;
        inventoryLabelX = ZombieFarmScreenLayout.INVENTORY_LABEL_X;
        inventoryLabelY = ZombieFarmScreenLayout.INVENTORY_LABEL_Y;
    }
    ...
    protected void init() {
        ...
        lootHelpButton = addRenderableWidget(Button.builder(Component.literal("?"), button -> {
                ...
                .bounds(
                        leftPos + ZombieFarmLootHelpPanel.BUTTON_X,
                        topPos + ZombieFarmLootHelpPanel.BUTTON_Y,
                        ZombieFarmLootHelpPanel.BUTTON_SIZE,
                        ZombieFarmLootHelpPanel.BUTTON_SIZE
                )
                .build());
        lootHelpButton.setTooltip(Tooltip.create(Component.translatable("button.trading_cells.zombie_loot_help")));
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        ZombieFarmScreenLayout.drawBackground(graphics, leftPos, topPos);
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ZombieFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.zombie_type"),
                9,
                115,
                8,
                18,
        ...
        ZombieFarmTextRenderer.left(
                graphics,
                font,
                playerInventoryTitle,
                ZombieFarmScreenLayout.INVENTORY_LABEL_X,
                338,
                106,
                12,
                TEXT_DARK,
        ...
        ZombieFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.zombie_loot"),
                FILTER_X,
        ...
        ZombieFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.zombie_outputs"),
                ZombieFarmMenu.OUTPUT_FIRST_X - 1,
                ZombieFarmMenu.OUTPUT_FIRST_X + ZombieFarmMenu.OUTPUT_COLUMNS * 18,
                ZombieFarmMenu.OUTPUT_FIRST_Y - 13,
                12,
                TEXT_DARK,
                false
        );
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (lootHelp.isOpen()) {
            if (inside(
                    x,
                    y,
                    ZombieFarmLootHelpPanel.BUTTON_X,
                    ZombieFarmLootHelpPanel.BUTTON_Y,
                    ZombieFarmLootHelpPanel.BUTTON_SIZE,
                    ZombieFarmLootHelpPanel.BUTTON_SIZE
            )) {
                lootHelp.toggle();
        ...
        if (kindListOpen) {
            if (inside(x, y, KIND_LIST_X, KIND_LIST_Y, SELECTOR_WIDTH, visibleKindCount() * KIND_ROW_HEIGHT)) {
                ...
                if (selected < menu.targetEntries().size()) {
                    sendButton(ZombieFarmMenu.SELECT_KIND_BUTTON_BASE + selected);
                    lootScroll = 0;
                }
                kindListOpen = false;
                return true;
        ...
        if (inside(x, y, FILTER_X, FILTER_Y, FILTER_WIDTH, VISIBLE_FILTERS * FILTER_ROW_HEIGHT)) {
            ...
            if (index < availableLoot.size()) {
                ZombieFarmLoot loot = availableLoot.get(index);
                sendButton(ZombieFarmMenu.TOGGLE_LOOT_BUTTON_BASE + loot.ordinal());
            } else {
                int dynamicIndex = index - availableLoot.size();
                if (dynamicIndex < menu.dynamicLootOptions().size()) {
                    sendButton(ZombieFarmMenu.TOGGLE_DYNAMIC_LOOT_BUTTON_BASE + dynamicIndex);
                }
            }
            return true;
        }
        if (inside(x, y, XP_BUTTON_X, XP_BUTTON_Y, XP_BUTTON_WIDTH, XP_BUTTON_HEIGHT)) {
            sendButton(ZombieFarmMenu.EXTRACT_EXPERIENCE_BUTTON);
            return true;
        }
        if (inside(x, y, POWER_X, POWER_Y, POWER_WIDTH, POWER_HEIGHT)) {
            sendButton(ZombieFarmMenu.TOGGLE_ENABLED_BUTTON);
            return true;
        }
    ...
    private void drawInventorySlots(GuiGraphicsExtractor graphics) {
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                SlotRenderer.drawAtItemPosition(
                        ...
                        leftPos,
                        topPos,
                        ZombieFarmMenu.PLAYER_INVENTORY_X + column * 18,
                        ZombieFarmMenu.PLAYER_INVENTORY_Y + row * 18,
                        COLORS.slotPalette()
                );
            }
        }
        for (int column = 0; column < 9; column++) {
            SlotRenderer.drawAtItemPosition(
                    ...
                    topPos,
                    ZombieFarmMenu.PLAYER_INVENTORY_X + column * 18,
                    ZombieFarmMenu.PLAYER_HOTBAR_Y,
                    COLORS.slotPalette()
            );
        }
        int[] equipmentY = {
                ZombieFarmMenuLayout.EQUIPMENT_HEAD_Y,
                ZombieFarmMenuLayout.EQUIPMENT_CHEST_Y,
                ZombieFarmMenuLayout.EQUIPMENT_LEGS_Y,
                ZombieFarmMenuLayout.EQUIPMENT_FEET_Y,
                ZombieFarmMenuLayout.EQUIPMENT_OFFHAND_Y
        };
        for (int frameY : equipmentY) {
            ZombieFarmScreenLayout.drawSlotAtFramePosition(
                    graphics,
                    leftPos,
                    topPos,
                    ZombieFarmMenuLayout.EQUIPMENT_X,
                    frameY,
                    COLORS
            );
        }
    ...
    private void drawMachineSlots(GuiGraphicsExtractor graphics) {
        drawItemSlot(graphics, ZombieFarmMenu.WORKER_SLOT_X, ZombieFarmMenu.INPUT_SLOT_Y);
        drawItemSlot(graphics, ZombieFarmMenu.SWORD_SLOT_X, ZombieFarmMenu.INPUT_SLOT_Y);
        if (!menu.getSlot(ZombieFarmBlockEntity.WORKER_SLOT).hasItem()) {
            graphics.blitSprite(
                    RenderPipelines.GUI_TEXTURED,
                    MachineSlotSprites.CAPTURER,
                    leftPos + ZombieFarmMenu.WORKER_SLOT_X,
                    topPos + ZombieFarmMenu.INPUT_SLOT_Y,
                    16,
                    16
            );
        }
        if (!menu.getSlot(ZombieFarmBlockEntity.SWORD_SLOT).hasItem()) {
            int x = leftPos + ZombieFarmMenu.SWORD_SLOT_X;
            int y = topPos + ZombieFarmMenu.INPUT_SLOT_Y;
            graphics.fakeItem(EMPTY_SWORD_PREVIEW, x, y);
            graphics.fill(x, y, x + 16, y + 16, 0x90606060);
        }
        for (int index = 0; index < ZombieFarmBlockEntity.OUTPUT_SLOT_COUNT; index++) {
            drawItemSlot(graphics, ZombieFarmMenu.outputSlotX(index), ZombieFarmMenu.outputSlotY(index));
        }
    }
    ...
    private void drawExperience(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        ZombieFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("button.trading_cells.zombie_withdraw_xp"),
    ...
    private void drawPowerButton(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        ZombieFarmTextRenderer.centered(
                ...
                Component.translatable(menu.isEnabled()
                        ? "gui.trading_cells.zombie_farm.active"
                        : "gui.trading_cells.zombie_farm.paused"),
                x + 21,
                x + POWER_WIDTH - 4,
                y + 1,
                POWER_HEIGHT - 2,
    ...
    private void drawLootFilters(GuiGraphicsExtractor graphics) {
        ...
        for (int row = 0; row < visible; row++) {
            ...
            ZombieFarmLoot loot = staticOption ? availableLoot.get(index) : null;
    ...
    private static Component lootName(ZombieFarmLoot loot) {
        return Component.translatable(
                "gui.trading_cells.zombie_loot." + loot.name().toLowerCase(java.util.Locale.ROOT)
        );
    }
