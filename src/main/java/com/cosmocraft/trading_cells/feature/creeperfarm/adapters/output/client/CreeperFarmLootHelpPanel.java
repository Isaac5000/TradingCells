...
/** Contextual live-yield panel owned by the normal Creeper Farm screen, never by REI. */
final class CreeperFarmLootHelpPanel {
    ...
    private int scroll;
    private CreeperFarmKind cachedKind;
    private int cachedLootMask = Integer.MIN_VALUE;
    ...
    private int cachedDecapitation = Integer.MIN_VALUE;
    ...
    void tick(CreeperFarmMenu menu) {
        refresh(menu);
    ...
    void drawPanel(
            ...
            Font font,
            CreeperFarmMenu menu,
            int left,
            int top,
            int mouseX,
            int mouseY,
            CreeperFarmGuiThemeColors colors
    ) {
        refresh(menu);
        int x = left + PANEL_X;
        int y = top + PANEL_Y;
        ...
        CreeperFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.creeper_help.title"),
                x + 28,
                x + PANEL_WIDTH - 28,
                y + 2,
                HEADER_HEIGHT - 2,
        ...
        if (entries.isEmpty()) {
            CreeperFarmTextRenderer.centered(
                    graphics,
                    font,
                    Component.translatable("gui.trading_cells.creeper_help.no_loot"),
                    x + PANEL_PADDING,
                    x + PANEL_WIDTH - PANEL_PADDING,
                    firstRowY + 10,
                    ROW_HEIGHT,
    ...
    private void refresh(CreeperFarmMenu menu) {
        CreeperFarmKind kind = menu.selectedKind();
        int lootMask = menu.enabledLootMask();
        ...
        int decapitation = menu.decapitationLevel();
        int dynamicState = dynamicState(menu);
        if (kind == cachedKind
                && lootMask == cachedLootMask
                && looting == cachedLooting
                && kills == cachedKills
                && enabled == cachedEnabled
                && worker == cachedWorker
                && sword == cachedSword
                && decapitation == cachedDecapitation
                && dynamicState == cachedDynamicState) {
            return;
        }
        cachedKind = kind;
        cachedLootMask = lootMask;
        cachedLooting = looting;
        cachedKills = kills;
        cachedEnabled = enabled;
        cachedWorker = worker;
        cachedSword = sword;
        cachedDecapitation = decapitation;
        cachedDynamicState = dynamicState;

        boolean previewAvailable = sword;
        List<Entry> refreshed = new ArrayList<>();
        for (CreeperFarmLootAdapter.PreviewOutput preview : CreeperFarmLootAdapter.previewOutputChances(kind)) {
            if (!menu.isLootEnabled(preview.loot())) {
                continue;
            }
            CreeperFarmDropRules.BaseDrop drop = CreeperFarmLootAdapter.currentCycleDrop(
                    kind,
                    preview.loot(),
                    preview.stack(),
                    looting,
            ...
            boolean dropAvailable = previewAvailable && drop.probabilityPartsPerMillion() > 0;
            refreshed.add(new Entry(
                    preview.stack(),
                    dropAvailable ? drop.probabilityPartsPerMillion() : 0,
                    dropAvailable ? drop.minimumAmount() : 0,
                    dropAvailable ? drop.maximumAmount() : 0,
                    false
            ));
        }
    ...
    private static int dynamicState(CreeperFarmMenu menu) {
    ...
    private static Component status(
        ...
        if (!enabled) {
            return Component.translatable("gui.trading_cells.creeper_help.paused");
        }
        if (!worker) {
            return Component.translatable("gui.trading_cells.creeper_help.missing_villager");
        }
        if (!sword) {
            return Component.translatable("gui.trading_cells.creeper_help.missing_sword");
        }
        if (empty) {
            return Component.translatable("gui.trading_cells.creeper_help.no_loot");
        }
        return Component.translatable("gui.trading_cells.creeper_help.stats", looting, kills);
    }
        ...
        if (hovered) {
            Component amountTooltip = entry.minimumAmount() == entry.maximumAmount()
                    ? Component.translatable(
                            "gui.trading_cells.creeper_help.amount_exact",
                            entry.maximumAmount()
                    )
                    : Component.translatable(
                            "gui.trading_cells.creeper_help.amount_range",
                            entry.minimumAmount(),
                            entry.maximumAmount()
                    );
            List<Component> tooltip = entry.dynamic()
                    ? List.of(
                            entry.stack().getHoverName(),
                            Component.translatable("gui.trading_cells.creeper_help.dynamic")
                    )
                    : List.of(
                            entry.stack().getHoverName(),
                            Component.translatable("gui.trading_cells.creeper_help.chance", probability),
                            amountTooltip
                    );
