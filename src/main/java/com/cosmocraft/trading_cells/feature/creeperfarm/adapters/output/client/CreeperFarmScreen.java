...
import com.cosmocraft.trading_cells.feature.creeperfarm.adapters.input.CreeperFarmMenuLayout;
...
public final class CreeperFarmScreen extends AbstractContainerScreen<CreeperFarmMenu> {
    ...
    private static final CreeperFarmGuiThemeColors COLORS = CreeperFarmGuiThemeColors.resolve();
    ...
    private final CreeperFarmLootHelpPanel lootHelp = new CreeperFarmLootHelpPanel();
    private Button lootHelpButton;
    private boolean kindListOpen;
    private int kindScroll;
    private int lootScroll;

    public CreeperFarmScreen(CreeperFarmMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, CreeperFarmMenu.WIDTH, CreeperFarmMenu.HEIGHT);
        titleLabelY = CreeperFarmScreenLayout.HEADER_TEXT_Y;
        inventoryLabelX = CreeperFarmScreenLayout.INVENTORY_LABEL_X;
        inventoryLabelY = CreeperFarmScreenLayout.INVENTORY_LABEL_Y;
    }
    ...
    protected void init() {
        ...
        lootHelpButton = addRenderableWidget(Button.builder(Component.literal("?"), button -> {
                ...
                .bounds(
                        leftPos + CreeperFarmLootHelpPanel.BUTTON_X,
                        topPos + CreeperFarmLootHelpPanel.BUTTON_Y,
                        CreeperFarmLootHelpPanel.BUTTON_SIZE,
                        CreeperFarmLootHelpPanel.BUTTON_SIZE
                )
                .build());
        lootHelpButton.setTooltip(Tooltip.create(Component.translatable("button.trading_cells.creeper_loot_help")));
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        CreeperFarmScreenLayout.drawBackground(graphics, leftPos, topPos);
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        CreeperFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.creeper_type"),
                9,
                115,
                8,
                18,
        ...
        CreeperFarmTextRenderer.left(
                graphics,
                font,
                playerInventoryTitle,
                CreeperFarmScreenLayout.INVENTORY_LABEL_X,
                338,
                106,
                12,
                TEXT_DARK,
        ...
        CreeperFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.creeper_loot"),
                FILTER_X,
        ...
        CreeperFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.creeper_outputs"),
                CreeperFarmMenu.OUTPUT_FIRST_X - 1,
                CreeperFarmMenu.OUTPUT_FIRST_X + CreeperFarmMenu.OUTPUT_COLUMNS * 18,
                CreeperFarmMenu.OUTPUT_FIRST_Y - 11,
                12,
                TEXT_DARK,
                false
        );
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (lootHelp.isOpen()) {
            if (inside(
                    x,
                    y,
                    CreeperFarmLootHelpPanel.BUTTON_X,
                    CreeperFarmLootHelpPanel.BUTTON_Y,
                    CreeperFarmLootHelpPanel.BUTTON_SIZE,
                    CreeperFarmLootHelpPanel.BUTTON_SIZE
            )) {
                lootHelp.toggle();
        ...
        if (kindListOpen) {
            if (inside(x, y, KIND_LIST_X, KIND_LIST_Y, SELECTOR_WIDTH, visibleKindCount() * KIND_ROW_HEIGHT)) {
                ...
                if (selected < menu.targetEntries().size()) {
                    sendButton(CreeperFarmMenu.SELECT_KIND_BUTTON_BASE + selected);
                    lootScroll = 0;
                }
                kindListOpen = false;
                return true;
        ...
        if (inside(x, y, FILTER_X, FILTER_Y, FILTER_WIDTH, VISIBLE_FILTERS * FILTER_ROW_HEIGHT)) {
            ...
            if (index < availableLoot.size()) {
                CreeperFarmLoot loot = availableLoot.get(index);
                sendButton(CreeperFarmMenu.TOGGLE_LOOT_BUTTON_BASE + loot.ordinal());
            } else {
                int dynamicIndex = index - availableLoot.size();
                if (dynamicIndex < menu.dynamicLootOptions().size()) {
                    sendButton(CreeperFarmMenu.TOGGLE_DYNAMIC_LOOT_BUTTON_BASE + dynamicIndex);
                }
            }
            return true;
        }
        if (inside(x, y, XP_BUTTON_X, XP_BUTTON_Y, XP_BUTTON_WIDTH, XP_BUTTON_HEIGHT)) {
            sendButton(CreeperFarmMenu.EXTRACT_EXPERIENCE_BUTTON);
            return true;
        }
        if (inside(x, y, POWER_X, POWER_Y, POWER_WIDTH, POWER_HEIGHT)) {
            sendButton(CreeperFarmMenu.TOGGLE_ENABLED_BUTTON);
            return true;
        }
    ...
    private void drawInventorySlots(GuiGraphicsExtractor graphics) {
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                SlotRenderer.drawAtItemPosition(
                        ...
                        leftPos,
                        topPos,
                        CreeperFarmMenu.PLAYER_INVENTORY_X + column * 18,
                        CreeperFarmMenu.PLAYER_INVENTORY_Y + row * 18,
                        COLORS.slotPalette()
                );
            }
        }
        for (int column = 0; column < 9; column++) {
            SlotRenderer.drawAtItemPosition(
                    ...
                    topPos,
                    CreeperFarmMenu.PLAYER_INVENTORY_X + column * 18,
                    CreeperFarmMenu.PLAYER_HOTBAR_Y,
                    COLORS.slotPalette()
            );
        }
        int[] equipmentY = {
                CreeperFarmMenuLayout.EQUIPMENT_HEAD_Y,
                CreeperFarmMenuLayout.EQUIPMENT_CHEST_Y,
                CreeperFarmMenuLayout.EQUIPMENT_LEGS_Y,
                CreeperFarmMenuLayout.EQUIPMENT_FEET_Y,
                CreeperFarmMenuLayout.EQUIPMENT_OFFHAND_Y
        };
        for (int frameY : equipmentY) {
            CreeperFarmScreenLayout.drawSlotAtFramePosition(
                    graphics,
                    leftPos,
                    topPos,
                    CreeperFarmMenuLayout.EQUIPMENT_X,
                    frameY,
                    COLORS
            );
        }
    ...
    private void drawMachineSlots(GuiGraphicsExtractor graphics) {
        drawItemSlot(graphics, CreeperFarmMenu.WORKER_SLOT_X, CreeperFarmMenu.INPUT_SLOT_Y);
        drawItemSlot(graphics, CreeperFarmMenu.SWORD_SLOT_X, CreeperFarmMenu.INPUT_SLOT_Y);
        if (!menu.getSlot(CreeperFarmBlockEntity.WORKER_SLOT).hasItem()) {
            graphics.blitSprite(
                    RenderPipelines.GUI_TEXTURED,
                    MachineSlotSprites.CAPTURER,
                    leftPos + CreeperFarmMenu.WORKER_SLOT_X,
                    topPos + CreeperFarmMenu.INPUT_SLOT_Y,
                    16,
                    16
            );
        }
        if (!menu.getSlot(CreeperFarmBlockEntity.SWORD_SLOT).hasItem()) {
            int x = leftPos + CreeperFarmMenu.SWORD_SLOT_X;
            int y = topPos + CreeperFarmMenu.INPUT_SLOT_Y;
        ...
        for (int index = 0; index < CreeperFarmBlockEntity.OUTPUT_SLOT_COUNT; index++) {
            drawItemSlot(graphics, CreeperFarmMenu.outputSlotX(index), CreeperFarmMenu.outputSlotY(index));
        }
    }
    ...
    private void drawExperience(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        CreeperFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("button.trading_cells.creeper_withdraw_xp"),
    ...
    private void drawPowerButton(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        CreeperFarmTextRenderer.centered(
                ...
                Component.translatable(menu.isEnabled()
                        ? "gui.trading_cells.creeper_farm.active"
                        : "gui.trading_cells.creeper_farm.paused"),
                x + 21,
                x + POWER_WIDTH - 4,
                y + 1,
                POWER_HEIGHT - 2,
    ...
    private void drawLootFilters(GuiGraphicsExtractor graphics) {
        ...
        for (int row = 0; row < visible; row++) {
            ...
            CreeperFarmLoot loot = staticOption ? availableLoot.get(index) : null;
    ...
    private static Component lootName(CreeperFarmLoot loot) {
        return Component.translatable(
                "gui.trading_cells.creeper_loot." + loot.name().toLowerCase(java.util.Locale.ROOT)
        );
    }
