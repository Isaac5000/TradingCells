...
/** Contextual live-yield panel owned by the normal Raider Farm screen, never by REI. */
final class RaiderFarmLootHelpPanel {
    ...
    private int scroll;
    private RaiderFarmKind cachedKind;
    ...
    private void refresh(RaiderFarmMenu menu) {
        RaiderFarmKind kind = menu.selectedKind();
        ...
        for (RaiderFarmLootAdapter.PreviewOutput preview : RaiderFarmLootAdapter.previewOutputChances(
            ...
            RaiderFarmDropRules.BaseDrop drop = RaiderFarmLootAdapter.currentCycleDrop(
                    kind,
                    preview.loot(),
                    preview.stack(),
                    looting,
        ...
        for (ItemStack stack : menu.dynamicLootOptions()) {
            if (menu.isDynamicLootEnabled(stack)) {
                var knownDrop = RaiderFarmLootAdapter.currentDynamicCycleDrop(
                        menu.selectedTargetId(),
                        stack,
                        looting,
                        kills
                ...
                if (knownDrop.isPresent() && previewAvailable) {
                    RaiderFarmDropRules.BaseDrop drop = knownDrop.orElseThrow();
                    refreshed.add(new Entry(
                            stack,
    ...
    private static int dynamicState(RaiderFarmMenu menu) {
