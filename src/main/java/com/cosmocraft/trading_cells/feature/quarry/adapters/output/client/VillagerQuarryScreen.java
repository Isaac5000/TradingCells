...
public final class VillagerQuarryScreen extends AbstractContainerScreen<QuarryMenu> {
    ...
    private static final Identifier BACKGROUND = Identifier.fromNamespaceAndPath(
            TradingCells.MOD_ID,
            "textures/gui/quarry/villager_background.png"
    );
    ...
    private static final Identifier SURFACE_OVERLAY = Identifier.fromNamespaceAndPath(
            TradingCells.MOD_ID,
            "textures/gui/quarry/villager_surface_overlay.png"
    );
    ...
    private static final int FRAME_DARK = 0xFF34383B;
    private static final int FRAME_LIGHT = 0xFFAEB5B9;
    ...
    private static final int DEEP_X = 10;
    private static final int DEEP_Y = 186;
    private static final int DEEP_WIDTH = 103;
    private static final int DEEP_HEIGHT = 18;
    ...
    private static final int ACCENT = 0xFF708591;
    ...
    public VillagerQuarryScreen(QuarryMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, VillagerQuarryMenuLayout.WIDTH, VillagerQuarryMenuLayout.HEIGHT);
        titleLabelY = 8;
        inventoryLabelX = 164;
        inventoryLabelY = 108;
    }
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawDeepMining(graphics, mouseX, mouseY);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (!catalog.isOpen() && inside(event.x(), event.y(), DEEP_X, DEEP_Y, DEEP_WIDTH, DEEP_HEIGHT)) {
            toggleDeepMining();
            return true;
        }
    ...
    private void drawBackground(GuiGraphicsExtractor graphics) {
        graphics.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, leftPos, topPos, 0.0F, 0.0F,
                VillagerQuarryMenuLayout.WIDTH, VillagerQuarryMenuLayout.HEIGHT,
                VillagerQuarryMenuLayout.ATLAS_WIDTH, VillagerQuarryMenuLayout.ATLAS_HEIGHT);
    }
    ...
    private void drawInventorySlots(GuiGraphicsExtractor graphics) {
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                drawSlot(graphics, VillagerQuarryMenuLayout.PLAYER_INVENTORY_X + column * 18,
                        VillagerQuarryMenuLayout.PLAYER_INVENTORY_Y + row * 18);
            }
        }
        for (int column = 0; column < 9; column++) {
            drawSlot(graphics, VillagerQuarryMenuLayout.PLAYER_INVENTORY_X + column * 18,
        ...
        drawSlot(graphics, VillagerQuarryMenuLayout.EQUIPMENT_X, VillagerQuarryMenuLayout.EQUIPMENT_HEAD_Y);
        drawSlot(graphics, VillagerQuarryMenuLayout.EQUIPMENT_X, VillagerQuarryMenuLayout.EQUIPMENT_CHEST_Y);
        drawSlot(graphics, VillagerQuarryMenuLayout.EQUIPMENT_X, VillagerQuarryMenuLayout.EQUIPMENT_LEGS_Y);
        drawSlot(graphics, VillagerQuarryMenuLayout.EQUIPMENT_X, VillagerQuarryMenuLayout.EQUIPMENT_FEET_Y);
        drawSlot(graphics, VillagerQuarryMenuLayout.EQUIPMENT_X, VillagerQuarryMenuLayout.EQUIPMENT_OFFHAND_Y);
    }

    private void drawMachineSlots(GuiGraphicsExtractor graphics) {
        drawSlot(graphics, VillagerQuarryMenuLayout.WORKER_SLOT_X, VillagerQuarryMenuLayout.WORKER_SLOT_Y);
        drawSlot(graphics, VillagerQuarryMenuLayout.PICKAXE_SLOT_X, VillagerQuarryMenuLayout.PICKAXE_SLOT_Y);
        drawSlot(graphics, VillagerQuarryMenuLayout.UPGRADE_SLOT_X, VillagerQuarryMenuLayout.UPGRADE_SLOT_Y);
        if (!menu.getSlot(QuarryBlockEntity.WORKER_SLOT).hasItem()) {
            drawEmptySprite(graphics, VillagerQuarryMenuLayout.WORKER_SLOT_X,
                    VillagerQuarryMenuLayout.WORKER_SLOT_Y, MachineSlotSprites.VILLAGER_HEAD);
        }
        if (!menu.getSlot(QuarryBlockEntity.PICKAXE_SLOT).hasItem()) {
            drawGhostItem(graphics, new ItemStack(Items.IRON_PICKAXE),
                    VillagerQuarryMenuLayout.PICKAXE_SLOT_X, VillagerQuarryMenuLayout.PICKAXE_SLOT_Y);
        }
        if (!menu.getSlot(QuarryBlockEntity.UPGRADE_SLOT).hasItem()) {
            drawGhostItem(graphics, QuarryRegistrationAdapter.QUARRY_COPPER_UPGRADE_ITEM.get().getDefaultInstance(),
                    VillagerQuarryMenuLayout.UPGRADE_SLOT_X, VillagerQuarryMenuLayout.UPGRADE_SLOT_Y);
        }
    ...
    private void drawProgress(GuiGraphicsExtractor graphics) {
        ...
        if (fill > 0) {
            graphics.fill(x + 2, y + 2, x + 2 + fill, y + RECIPE_VIEWER_HEIGHT - 2, 0xFF8AA6B7);
        }
    ...
    private void drawDeepMining(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        int x = leftPos + DEEP_X;
        int y = topPos + DEEP_Y;
        boolean available = menu.deepMiningAvailable();
        boolean hovered = inside(mouseX, mouseY, DEEP_X, DEEP_Y, DEEP_WIDTH, DEEP_HEIGHT);
        int fill = !available ? 0xFF686868 : hovered ? 0xFF78946A : 0xFF677F58;
        graphics.fill(x, y, x + DEEP_WIDTH, y + DEEP_HEIGHT, 0xFF383838);
        graphics.fill(x + 1, y + 1, x + DEEP_WIDTH - 1, y + DEEP_HEIGHT - 1, fill);
        int boxX = x + 4;
        int boxY = y + 3;
        graphics.fill(boxX, boxY, boxX + 11, boxY + 11, 0xFF272727);
        graphics.fill(boxX + 1, boxY + 1, boxX + 10, boxY + 10, available ? 0xFFB7B7B7 : 0xFF858585);
        if (menu.deepMining()) {
            drawCheckmark(graphics, boxX, boxY);
        }
        FittedTextRenderer.centered(graphics, font, Component.translatable("button.trading_cells.deep_mining"),
                x + 19, x + DEEP_WIDTH - 3, y, DEEP_HEIGHT, 0xFFFFFFFF, false);
        if (hovered) {
            graphics.setTooltipForNextFrame(
                    font,
                    Component.translatable(available
                            ? "button.trading_cells.deep_mining"
                            : "tooltip.trading_cells.deep_mining_requires_diamond"),
                    mouseX,
                    mouseY
            );
        }
    ...
    private static void drawProgressFrame(GuiGraphicsExtractor graphics, int x, int y) {
        graphics.fill(x, y, x + RECIPE_VIEWER_WIDTH, y + RECIPE_VIEWER_HEIGHT, FRAME_DARK);
        graphics.fill(x + 1, y + 1, x + RECIPE_VIEWER_WIDTH - 1, y + RECIPE_VIEWER_HEIGHT - 1, FRAME_LIGHT);
