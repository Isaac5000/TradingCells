...
/** Public combat boundary used by every mob-farm family and global combat event. */
public final class CombatEnchantments {
    ...
    public static boolean protectsWeapon(ItemStack stack, HolderLookup.Provider registries) {
    ...
    public static double effectiveDamageLevel(
        ...
        double fallback = sharpnessEquivalentLevel(sharpness);
        if (stack.isEmpty()) {
            return fallback;
        }

        Entity target = targetType.create(level, EntitySpawnReason.LOAD);
    ...
    public static boolean isWarriorsTouchStoredOnBook(ItemStack stack, HolderLookup.Provider registries) {
