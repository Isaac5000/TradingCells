...
/** Contextual live-yield panel owned by the normal Skeleton Farm screen, never by REI. */
final class SkeletonFarmLootHelpPanel {
    ...
    private int scroll;
    private SkeletonFarmKind cachedKind;
    ...
    void tick(SkeletonFarmMenu menu) {
        refresh(menu);
    ...
    void drawPanel(
        ...
        SkeletonFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.skeleton_help.title"),
                x + 28,
                x + PANEL_WIDTH - 28,
                y + 2,
                HEADER_HEIGHT - 2,
        ...
        if (entries.isEmpty()) {
            SkeletonFarmTextRenderer.centered(
                    graphics,
                    font,
                    Component.translatable("gui.trading_cells.skeleton_help.no_loot"),
                    x + PANEL_PADDING,
                    x + PANEL_WIDTH - PANEL_PADDING,
                    firstRowY + 10,
                    ROW_HEIGHT,
    ...
    private void refresh(SkeletonFarmMenu menu) {
        SkeletonFarmKind kind = menu.selectedKind();
        ...
        for (SkeletonFarmLootAdapter.PreviewOutput preview : SkeletonFarmLootAdapter.previewOutputChances(kind)) {
            if (!menu.isLootEnabled(preview.loot())) {
                continue;
            }
            SkeletonFarmDropRules.BaseDrop drop = preview.loot() == SkeletonFarmLoot.SKULLS
                    ? SkeletonFarmDropRules.headCycleDrop(kind, looting, kills, decapitation)
                    : SkeletonFarmDropRules.cycleDrop(preview.loot(), looting, kills);
    ...
    private static int dynamicState(SkeletonFarmMenu menu) {
    ...
    private static Component status(
        ...
        if (!enabled) {
            return Component.translatable("gui.trading_cells.skeleton_help.paused");
        }
        if (!worker) {
            return Component.translatable("gui.trading_cells.skeleton_help.missing_villager");
        }
        if (!sword) {
            return Component.translatable("gui.trading_cells.skeleton_help.missing_sword");
        }
        if (empty) {
            return Component.translatable("gui.trading_cells.skeleton_help.no_loot");
        }
        return Component.translatable("gui.trading_cells.skeleton_help.stats", looting, kills);
    }
    ...
    private static void drawRow(
        ...
        if (hovered) {
            ...
            List<Component> tooltip = entry.dynamic()
                    ? List.of(
                            entry.stack().getHoverName(),
                            Component.translatable("gui.trading_cells.skeleton_help.dynamic")
                    )
                    : List.of(
                            entry.stack().getHoverName(),
                            Component.translatable("gui.trading_cells.skeleton_help.chance", probability),
                            amountTooltip
                    );
