...
/** Stable preview math for special configurable-farm drops. Real drops still use loaded loot tables. */
public final class ConfiguredMobFarmDropRules {
    ...
    private ConfiguredMobFarmDropRules() {
    }
    ...
    public static double spawnedEquipmentChance(double spawnChance, int lootingLevel) {
        return Math.clamp(spawnChance, 0.0D, 1.0D) * equipmentDropChance(lootingLevel);
    }

    public static double weaponChance(int lootingLevel) {
        return equipmentDropChance(lootingLevel);
    }
    ...
    public record BaseDrop(int probabilityPartsPerMillion, int minimumAmount, int maximumAmount) {
        public BaseDrop {
            if (probabilityPartsPerMillion < 0
                ...
                throw new IllegalArgumentException("Invalid configurable mob-farm drop preview");
            }
        }
    }
}
