...
import com.cosmocraft.trading_cells.feature.configuredmobfarm.adapters.input.ConfiguredMobFarmMenuLayout;
...
public final class ConfiguredMobFarmScreen extends AbstractContainerScreen<ConfiguredMobFarmMenu> {
    ...
    private static final ConfiguredMobFarmGuiThemeColors COLORS = ConfiguredMobFarmGuiThemeColors.resolve();
    ...
    private final ConfiguredMobFarmLootHelpPanel lootHelp = new ConfiguredMobFarmLootHelpPanel();
    private Button lootHelpButton;
    private boolean kindListOpen;
    private int kindScroll;
    private int lootScroll;

    public ConfiguredMobFarmScreen(ConfiguredMobFarmMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, ConfiguredMobFarmMenu.WIDTH, ConfiguredMobFarmMenu.HEIGHT);
        titleLabelY = ConfiguredMobFarmScreenLayout.HEADER_TEXT_Y;
        inventoryLabelX = ConfiguredMobFarmScreenLayout.INVENTORY_LABEL_X;
        inventoryLabelY = ConfiguredMobFarmScreenLayout.INVENTORY_LABEL_Y;
    }
    ...
    protected void init() {
        ...
        lootHelpButton = addRenderableWidget(Button.builder(Component.literal("?"), button -> {
                ...
                .bounds(
                        leftPos + ConfiguredMobFarmLootHelpPanel.BUTTON_X,
                        topPos + ConfiguredMobFarmLootHelpPanel.BUTTON_Y,
                        ConfiguredMobFarmLootHelpPanel.BUTTON_SIZE,
                        ConfiguredMobFarmLootHelpPanel.BUTTON_SIZE
                )
                .build());
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        ConfiguredMobFarmScreenLayout.drawBackground(graphics, leftPos, topPos);
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ConfiguredMobFarmTextRenderer.centered(
                graphics,
                font,
                Component.translatable("gui.trading_cells.mob_farm_type"),
                9,
                115,
                8,
                18,
        ...
        ConfiguredMobFarmTextRenderer.left(
                ...
                ConfiguredMobFarmScreenLayout.INVENTORY_LABEL_X,
                338,
                106,
                12,
                TEXT_DARK,
        ...
        ConfiguredMobFarmTextRenderer.centered(
                ...
                ConfiguredMobFarmMenu.OUTPUT_FIRST_X - 1,
                ConfiguredMobFarmMenu.OUTPUT_FIRST_X + ConfiguredMobFarmMenu.OUTPUT_COLUMNS * 18,
                ConfiguredMobFarmMenu.OUTPUT_FIRST_Y - 11,
                12,
                TEXT_DARK,
                false
        );
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (lootHelp.isOpen()) {
            if (inside(
                    x,
                    y,
                    ConfiguredMobFarmLootHelpPanel.BUTTON_X,
                    ConfiguredMobFarmLootHelpPanel.BUTTON_Y,
                    ConfiguredMobFarmLootHelpPanel.BUTTON_SIZE,
                    ConfiguredMobFarmLootHelpPanel.BUTTON_SIZE
            )) {
                lootHelp.toggle();
        ...
        if (kindListOpen) {
            if (inside(x, y, KIND_LIST_X, KIND_LIST_Y, SELECTOR_WIDTH, visibleKindCount() * KIND_ROW_HEIGHT)) {
                ...
                if (selected < menu.targetEntries().size()) {
                    sendButton(ConfiguredMobFarmMenu.SELECT_KIND_BUTTON_BASE + selected);
                    lootScroll = 0;
                }
                kindListOpen = false;
                return true;
        ...
        if (inside(x, y, FILTER_X, FILTER_Y, FILTER_WIDTH, VISIBLE_FILTERS * FILTER_ROW_HEIGHT)) {
            ...
            if (index < availableLoot.size()) {
                ConfiguredMobFarmLoot loot = availableLoot.get(index);
                sendButton(ConfiguredMobFarmMenu.TOGGLE_LOOT_BUTTON_BASE + loot.ordinal());
            } else {
                int dynamicIndex = index - availableLoot.size();
                if (dynamicIndex < menu.dynamicLootOptions().size()) {
                    sendButton(ConfiguredMobFarmMenu.TOGGLE_DYNAMIC_LOOT_BUTTON_BASE + dynamicIndex);
                }
            }
            return true;
        }
        if (inside(x, y, XP_BUTTON_X, XP_BUTTON_Y, XP_BUTTON_WIDTH, XP_BUTTON_HEIGHT)) {
            sendButton(ConfiguredMobFarmMenu.EXTRACT_EXPERIENCE_BUTTON);
            return true;
        }
        if (inside(x, y, POWER_X, POWER_Y, POWER_WIDTH, POWER_HEIGHT)) {
            sendButton(ConfiguredMobFarmMenu.TOGGLE_ENABLED_BUTTON);
            return true;
        }
    ...
    private void drawInventorySlots(GuiGraphicsExtractor graphics) {
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                SlotRenderer.drawAtItemPosition(
                        ...
                        leftPos,
                        topPos,
                        ConfiguredMobFarmMenu.PLAYER_INVENTORY_X + column * 18,
                        ConfiguredMobFarmMenu.PLAYER_INVENTORY_Y + row * 18,
                        COLORS.slotPalette()
                );
            }
        }
        for (int column = 0; column < 9; column++) {
            SlotRenderer.drawAtItemPosition(
                    ...
                    topPos,
                    ConfiguredMobFarmMenu.PLAYER_INVENTORY_X + column * 18,
                    ConfiguredMobFarmMenu.PLAYER_HOTBAR_Y,
                    COLORS.slotPalette()
            );
        }
        int[] equipmentY = {
                ConfiguredMobFarmMenuLayout.EQUIPMENT_HEAD_Y,
                ConfiguredMobFarmMenuLayout.EQUIPMENT_CHEST_Y,
                ConfiguredMobFarmMenuLayout.EQUIPMENT_LEGS_Y,
                ConfiguredMobFarmMenuLayout.EQUIPMENT_FEET_Y,
                ConfiguredMobFarmMenuLayout.EQUIPMENT_OFFHAND_Y
        };
        for (int frameY : equipmentY) {
            ConfiguredMobFarmScreenLayout.drawSlotAtFramePosition(
                    graphics,
                    leftPos,
                    topPos,
                    ConfiguredMobFarmMenuLayout.EQUIPMENT_X,
                    frameY,
                    COLORS
            );
        }
    ...
    private void drawMachineSlots(GuiGraphicsExtractor graphics) {
        drawItemSlot(graphics, ConfiguredMobFarmMenu.WORKER_SLOT_X, ConfiguredMobFarmMenu.INPUT_SLOT_Y);
        drawItemSlot(graphics, ConfiguredMobFarmMenu.SWORD_SLOT_X, ConfiguredMobFarmMenu.INPUT_SLOT_Y);
        if (!menu.getSlot(ConfiguredMobFarmBlockEntity.WORKER_SLOT).hasItem()) {
            graphics.blitSprite(
                    RenderPipelines.GUI_TEXTURED,
                    MachineSlotSprites.CAPTURER,
                    leftPos + ConfiguredMobFarmMenu.WORKER_SLOT_X,
                    topPos + ConfiguredMobFarmMenu.INPUT_SLOT_Y,
                    16,
                    16
            );
        }
        if (!menu.getSlot(ConfiguredMobFarmBlockEntity.SWORD_SLOT).hasItem()) {
            int x = leftPos + ConfiguredMobFarmMenu.SWORD_SLOT_X;
            int y = topPos + ConfiguredMobFarmMenu.INPUT_SLOT_Y;
        ...
        for (int index = 0; index < ConfiguredMobFarmBlockEntity.OUTPUT_SLOT_COUNT; index++) {
            drawItemSlot(graphics, ConfiguredMobFarmMenu.outputSlotX(index), ConfiguredMobFarmMenu.outputSlotY(index));
        }
    }
    ...
    private void drawLootFilters(GuiGraphicsExtractor graphics) {
        ...
        for (int row = 0; row < visible; row++) {
            ...
            ConfiguredMobFarmLoot loot = staticOption ? availableLoot.get(index) : null;
    ...
    private static Component lootName(ConfiguredMobFarmLoot loot) {
