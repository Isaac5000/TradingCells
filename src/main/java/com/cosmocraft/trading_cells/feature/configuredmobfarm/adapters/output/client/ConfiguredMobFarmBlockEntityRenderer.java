...
public final class ConfiguredMobFarmBlockEntityRenderer implements BlockEntityRenderer<
        ConfiguredMobFarmBlockEntity,
        ConfiguredMobFarmBlockEntityRenderer.State
> {
    private static final float WORKER_SCALE = 0.27F;
    private static final float SPAWNER_SCALE = 0.27F;
    private static final float SPAWNER_ENTITY_MAX_SCALE = 0.13F;
    private static final float SPAWNER_ENTITY_MAX_EXTENT = 0.36F;
    private static final double SPAWNER_Y = 0.14D;
    private static final double SPAWNER_ENTITY_Y = SPAWNER_Y + SPAWNER_SCALE;
    private static final double GHAST_ENTITY_Y_OFFSET = 0.16D;
    ...
    private final Map<ConfiguredMobFarmBlockEntity, EntityCache> entityCaches = new WeakHashMap<>();
    private boolean spawnerReady;

    public ConfiguredMobFarmBlockEntityRenderer(BlockEntityRendererProvider.Context context) {
    ...
    public void extractRenderState(
            @NonNull ConfiguredMobFarmBlockEntity blockEntity,
            @NonNull State state,
            float partialTicks,
        ...
        state.worker = null;
        state.raider = null;
        state.raiderScale = SPAWNER_ENTITY_MAX_SCALE;
        state.raiderY = SPAWNER_ENTITY_Y;
    ...
    private static float fittedSpawnerEntityScale(Entity entity) {
        float largestDimension = Math.max(entity.getBbWidth(), entity.getBbHeight());
        if (largestDimension <= 0.0F) {
            return SPAWNER_ENTITY_MAX_SCALE;
        }
        return Math.min(SPAWNER_ENTITY_MAX_SCALE, SPAWNER_ENTITY_MAX_EXTENT / largestDimension);
    }

    private static float previewScaleMultiplier(ConfiguredMobFarmKind kind, Identifier targetId) {
        if (kind == ConfiguredMobFarmKind.SLIME) {
            return 2.0F;
        }
        if (kind == ConfiguredMobFarmKind.GHAST
                || Identifier.withDefaultNamespace("elder_guardian").equals(targetId)) {
            return 0.5F;
        }
        return 1.0F;
    }

    private static double previewYOffset(ConfiguredMobFarmKind kind) {
    ...
    public static final class State extends BlockEntityRenderState {
        ...
        public float raiderScale = SPAWNER_ENTITY_MAX_SCALE;
        public double raiderY = SPAWNER_ENTITY_Y;
    ...
    private final class EntityCache {
        ...
        private @Nullable Entity getOrCreateWorker(ConfiguredMobFarmBlockEntity blockEntity, Level level) {
            ItemStack workerStack = blockEntity.getItem(ConfiguredMobFarmBlockEntity.WORKER_SLOT);
        ...
        private @Nullable Entity getOrCreateRaider(Level level, Identifier targetId) {
            if (cachedRaider == null || !cachedTargetId.equals(targetId)) {
                Identifier entityTypeId = ConfiguredMobFarmTargetCatalog.entityTypeId(targetId);
                ...
                if (cachedRaider instanceof LivingEntity living) {
                    ItemStack weapon = ConfiguredMobFarmTargetCatalog.defaultWeapon(targetId);
