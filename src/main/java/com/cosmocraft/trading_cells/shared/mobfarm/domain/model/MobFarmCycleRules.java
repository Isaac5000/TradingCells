package com.cosmocraft.trading_cells.shared.mobfarm.domain.model;

import com.cosmocraft.trading_cells.shared.machines.domain.model.TimedProcess;

/** Pure timing and simulated-kill rules shared by independent mob-farm families. */
public final class MobFarmCycleRules {
    public static final double MAX_EFFECTIVE_DAMAGE_LEVEL = 5.0D;
    public static final int TICKS_PER_SECOND = 20;
    private static final double WOODEN_BASE_SECONDS = 120.0D;
    private static final double WOODEN_MAX_DAMAGE_SECONDS = 30.0D;
    private static final double NETHERITE_BASE_SECONDS = 20.0D;
    private static final double NETHERITE_MAX_DAMAGE_SECONDS = 5.0D;
    ...
    public static int effectiveCycleTicks(double tierPosition, double effectiveDamageLevel) {
        ...
        double maximumDamageDuration = maximumDamageDuration(position);
        ...
        double endingDistance = Math.max(
                Double.MIN_NORMAL,
                Math.min(startingDistance, maximumDamageDuration - MINIMUM_DURATION_SECONDS)
        );
    ...
    public static boolean isFilterEnabled(int enabledMask, int filterBit, boolean supported) {
        return supported && (enabledMask & filterBit) != 0;
    }

    public static int toggleFilter(int enabledMask, int filterBit) {
        return enabledMask ^ filterBit;
    }
    ...
    private static double maximumDamageDuration(double position) {
        if (position <= NETHERITE_POSITION) {
            return WOODEN_MAX_DAMAGE_SECONDS * Math.pow(TIER_RATIO, position);
        }
        return MINIMUM_DURATION_SECONDS
                + (NETHERITE_MAX_DAMAGE_SECONDS - MINIMUM_DURATION_SECONDS)
                * Math.pow(TIER_RATIO, position - NETHERITE_POSITION);
    }
}
