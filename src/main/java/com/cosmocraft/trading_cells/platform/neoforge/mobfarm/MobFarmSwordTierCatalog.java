...
/** Stable vanilla sword tiers extended at runtime with individually isolated modded swords. */
public final class MobFarmSwordTierCatalog {
    ...
    private static final List<Tier> VANILLA_TIERS = Arrays.stream(VanillaSwordTier.values())
            .map(MobFarmSwordTierCatalog::vanilla)
            .sorted(TIER_ORDER)
            .toList();
    ...
    private MobFarmSwordTierCatalog() {
    }
    ...
    private static Catalog discoverCatalog() {
        try {
            ...
            TradingCells.LOGGER.warn(
                    "The dynamic mob-farm sword tier catalog could not be assembled; only vanilla tiers will be used.",
                    exception
            );
            return VANILLA_CATALOG;
        }
